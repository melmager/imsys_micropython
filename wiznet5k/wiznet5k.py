...
disable DNS and DHCP - no need IMSYS has fix IP
remove Reset issue
"""
...
class WIZNET5K:  # pylint: disable=too-many-public-methods
    ...
    # pylint: disable=too-many-arguments - cange default setting
    def __init__(self, spi_bus, cs, reset=None, is_dhcp=False, mac=DEFAULT_MAC, hostname=None, dhcp_timeout=30, debug=False):
        self._debug = debug
        self._chip_type = None
        self._device = spi_bus
        self._device.init(baudrate=500000, polarity=0, phase=0) #baudrate=8000000
        self._cs = cs        
        # reset wiznet module prior to initialization (Active low)
        if reset:
            reset.on()
            time.sleep(0.1)
            reset.off()
    ...
    #remove set_dhcp, maintain_dhcp_lease, get_host_by_name
